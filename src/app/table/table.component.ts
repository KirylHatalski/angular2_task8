...
export class TableComponent implements OnInit, OnChanges, OnDestroy {
    ...
    options: string[] = ['farengate', 'kelven'];
    pattern: string = '';
    cityes: string[] = [];
    ...
    getCityList(pattern: string){
      this.cityes = []
      if(pattern.length < 3) return
      if(this.cityes.length) {
        this.tWeatherService.findCityName(pattern, this.cityes).then(data => {
          this.cityes = data;
        })
        return
      }
      this.tWeatherService.findCityName(pattern).then(data => {
        this.cityes = data;
      });
    }

    addCityByName(name: string){
      this.tWeatherService.getWeatherByName(name)
                          .then(data => {
                            this.weather.list.push(data)
                          })
